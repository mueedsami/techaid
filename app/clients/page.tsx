...
export default async function ClientsPage() {
  const { clients, testimonials } = await getPageData();

  const typeGroups = ["University", "Institute", "Industry", "Government", "Research", "Private"].reduce<
    ...
    <main className="overflow-x-hidden">
      <section className="relative px-6 sm:px-10 pt-20 pb-24">
        <div className="pointer-events-none absolute inset-0 overflow-hidden">
          ...
          <div
            className="absolute top-1/3 right-[10%] w-32 h-32 rounded-full border opacity-20 anim-float"
        ...
        <div className="relative mx-auto max-w-7xl">
          ...
          <h1 className="font-display text-[clamp(2.8rem,6vw,5.5rem)] font-semibold leading-[1.05] tracking-tight max-w-4xl">
            Trusted Across
            <br />
            <em className="shimmer-gold not-italic">Bangladesh&apos;s Leading</em>
            <br />
            Institutions
          </h1>
          <p className="mt-8 max-w-2xl text-lg leading-8 text-[var(--text-dim)]">
            Technical Aid supports a diverse range of clients with engineering
            equipment, technical services, installation, and long-term support.
          </p>
        ...
        <div className="mx-auto max-w-7xl grid grid-cols-2 md:grid-cols-4 gap-4">
          {[
            { label: "Total Clients", value: `${clients.length}+` },
            {
              ...
              value: `${
                (typeGroups["University"]?.length || 0) +
                (typeGroups["Institute"]?.length || 0)
              }+`,
            },
            {
              label: "Industry & Government",
              value: `${
                (typeGroups["Industry"]?.length || 0) +
                (typeGroups["Government"]?.length || 0)
              }+`,
            },
            { label: "Years of Service", value: "6+" },
          ].map((s) => (
            <div key={s.label} className="grad-border p-5 text-center">
              <p className="font-display text-3xl font-semibold shimmer-gold">
                {s.value}
              </p>
              <p className="mt-1 text-xs text-[var(--text-muted)] tracking-wide uppercase">
                {s.label}
              </p>
            </div>
          ))}
          ...
          <div className="mx-auto max-w-7xl">
            ...
            <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
              {items.map((client) => (
                <article
                  key={client.id}
                  className="card-lift grad-border p-5 flex flex-col gap-3"
                >
                  <div className="flex items-start justify-between gap-3">
                    <h3 className="font-semibold text-[var(--text)] text-sm leading-5">
                      {client.name}
                    </h3>
                    <span
                  ...
                  {client.sector && (
                    <p className="text-xs text-[var(--gold)] opacity-80">
                      {client.sector}
                    </p>
                  )}
                  {client.summary && (
        ...
        <div className="px-6 sm:px-10 pb-16">
          <div className="mx-auto max-w-7xl grad-border p-12 text-center">
            <p className="text-[var(--text-muted)]">
              Client information will be published soon.
            </p>
          </div>
        </div>
      )}
          ...
          <div className="mx-auto max-w-7xl">
            ...
            <div className="grid gap-5 md:grid-cols-2 lg:grid-cols-3">
              {testimonials.map((t) => (
                <div
                  key={t.id}
                  className="card-lift grad-border p-6 flex flex-col"
                >
      ...
      <section className="relative px-6 sm:px-10 py-24 overflow-hidden">
        ...
        <div className="relative mx-auto max-w-4xl text-center">
          <span
            className="gold-line block mb-6"
          ...
          <p className="mt-4 max-w-xl mx-auto text-[var(--text-dim)] leading-7">
            Whether you need sourcing, installation, training, or servicing
            support — Technical Aid is ready to help.
          </p>
